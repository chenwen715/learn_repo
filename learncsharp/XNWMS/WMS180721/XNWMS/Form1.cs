    ...
    public partial class Form1 : Form
    {
        ...
        private void InitUnLoadTask()
        {
            try
            {
                ...
                string selectString1 = string.Format(@"select * from dbo.T_Task_UnLoad where state=0 AND Times>={0} AND Times<{1} ", t - 10, t+10);
        ...
        private void ManageLoadData()
        {
            ...
            {
                if (schemaTable.Rows[n][2].ToString().Trim().EndsWith("$"))
                {
...
""opTime"":""{5}""}}]", task.taskNo, task.palletOrBoxNo, "[\"\"]", Convert.ToInt16(task.isHengWen), -1, DateTime.Now.ToString(), task.bdNo, task.pNo, task.weight, task.height);
        ...
        {     
            ...
            if (string.IsNullOrEmpty(task.palletOrBoxNo))
            {
                ...
                List<string> result = comnsql.ProcOutput(Properties.Settings.Default.sql, "[p_wms_insert_unloadtask_1]", paras, "@IsSplit", "@ShelfNo", "@PalletOrBoxNo");
                //task.isUnpackTray = Convert.ToBoolean(result[0]);
                isSplit = Convert.ToBoolean(result[0]);
                //task.hubno = result[1];
                //task.palletOrBoxNo = result[2];
                if (isSplit)
                {
                    InitUnLoadTask();
                    task.state = 1;

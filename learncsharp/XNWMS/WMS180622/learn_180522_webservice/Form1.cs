...
using learn_180522_webservice.InfoModel_S;
using System;
...
namespace learn_180522_webservice
{
    public partial class Form1 : Form
    {
        ...
        private void InitTimer()
        {
            ...
            threadTimer1 = new System.Threading.Timer(new TimerCallback(TimerUp1), null, 0, 1000);//下发任务给wcs
            ...
            for (int n = 0; n < schemaTable.Rows.Count; n++)
            {
                if (schemaTable.Rows[n][2].ToString().Trim().EndsWith("Unload$"))
                {
                    ...
                    for (int i = 0; i < totalNumber; i++)
                    {
                        ...
                        //string selectString = string.Format(@"SELECT top 1 ShelfNo, BoxNo FROM dbo.T_Biz_Container_BulksDtl WHERE BDNo = '{0}' AND PNo = '{1}'", unloadTask.bdNo, unloadTask.pNo);
                        //DataSet ds1 = comnsql.SelectGet(Properties.Settings.Default.sql, selectString);
                        //string selectString1 = string.Format(@"SELECT top 1 Warecell, PalletNo FROM dbo.T_Biz_Container_PalletDtl WHERE BDNo='{0}' AND PNo='{1}'", unloadTask.bdNo, unloadTask.pNo);
                        //DataSet ds2 = comnsql.SelectGet(Properties.Settings.Default.sql, selectString1);
                        //string sn="",shelf="";
                        //if(ds1.Tables[0].Rows.Count != 0||ds2.Tables[0].Rows.Count != 0)
                        //{
                        //    if (ds1.Tables[0].Rows.Count != 0)
                        //    {
                        //        DataRow drb = ds1.Tables[0].Rows[0];
                        //        sn = drb["BoxNo"].ToString();
                        //        shelf = drb["ShelfNo"].ToString();
                        //    }
                        //    else if(ds2.Tables[0].Rows.Count != 0)
                        //    {
                        //        DataRow drp = ds2.Tables[0].Rows[0];
                        //        sn = drp["BoxNo"].ToString();
                        //        shelf = drp["ShelfNo"].ToString();
                        //    }
...
""opTime"":""{9}""}}]", task.taskNo, task.taskType, task.taskState, task.priority, task.outNo, task.palletOrBoxNo, task.hubno, task.isPipeline, task.palletOrBoxNo, DateTime.Now.ToString());
        ...
        private void DownRemainTask()
        {
            ...
            int t = (DateTime.Now.Day - 1) * 86400 + DateTime.Now.Hour * 3600 + DateTime.Now.Minute * 60 + DateTime.Now.Second;
            //string selectString1 = string.Format(@"select * from dbo.T_Task_UnLoad where state=0 AND Times>={0} AND Times<{1} and isnull(HubNo,'')<>'' and isnull(SN,'')<>''", t, (DateTime.Now.Day + 1) * 86400);
            string selectString1 = string.Format(@"select * from dbo.T_Task_UnLoad where state=0 AND Times<{0} ", t);
            DataSet ds1 = comnsql.SelectGet(Properties.Settings.Default.sql, selectString1);
            if (ds1.Tables[0].Rows.Count != 0)
            {
                foreach (DataRow dr in ds1.Tables[0].Rows)
            ...
            int loadTaskNumber = 0;
            string selectString = string.Format(@"select * from dbo.T_Task_Load where state=0 AND Times<{0}", t);
            DataSet ds = comnsql.SelectGet(Properties.Settings.Default.sql, selectString);
            if (ds.Tables[0].Rows.Count != 0)
            {
                foreach (DataRow dr in ds.Tables[0].Rows)

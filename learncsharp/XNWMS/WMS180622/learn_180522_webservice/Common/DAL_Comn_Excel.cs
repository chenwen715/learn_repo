...
namespace learn_180522_webservice
{
    ...
    public class DAL_Comn_Excel
    {
        ...
        public Microsoft.Office.Interop.Excel.Workbook Open(string FileName)//打开一个Microsoft.Office.Interop.Excel文件
        {
            app = new Microsoft.Office.Interop.Excel.Application();
            wbs = app.Workbooks;
            //wb = wbs.Add(FileName);
        ...
        {//获取单元格内容
            string a = "";
            if (!string.IsNullOrEmpty(GetSheet(ws).Cells[x, y].Text))
            {
                a = GetSheet(ws).Cells[x, y].Text;
            }
            return a;
        }
        ...
        {//获取单元格内容
            string a = "";
            if (!string.IsNullOrEmpty(ws.Cells[x, y].Text))
            {
                a = ws.Cells[x, y].Text;
            }
            return a;

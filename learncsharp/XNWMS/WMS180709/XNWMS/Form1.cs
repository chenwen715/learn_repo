...
namespace XNWMS
{
    public partial class Form1 : Form
    {
        ...
        private void ManageLoadData()
        {
            ...
            {
                if (schemaTable.Rows[n][2].ToString().Trim().EndsWith("$") && schemaTable.Rows[n][2].ToString().Trim().Contains("Initia"))
                {
...
""opTime"":""{9}""}}]", task.taskNo, task.taskType, task.taskState, task.priority, task.outNo, task.palletOrBoxNo, task.hubno, task.isPipeline, task.palletOrBoxNo, DateTime.Now.ToString(),task.isUnpackTray,0,-1);
        ...
        private void UnLoadTasks()
        {
            ...
            string selectString = string.Format(@"SELECT TOP {0} Warecell, PalletNo, BDNo, PNo, IsOverWeight, IsHengWen
FROM dbo.T_Biz_Container_PalletDtl where flag=0 ORDER BY newid()",taskCount);
            ...
            if (ds.Tables[0].Rows.Count != 0)
            {
                ...
                foreach (DataRow dr in ds.Tables[0].Rows)
                {
                    ...
                    int n2 = r.Next(0, isUnpack.Length);
...
WHERE PalletNo = '{0}' AND Warecell='{1}' AND flag=0", task.palletOrBoxNo, task.hubno);
            ...
            comnlog.MessageLog("任务处理", DateTime.Now.ToString() + " 插入" + successNumber + "条整托下架任务完成" + "\n");
            InitUnLoadTask();
        }

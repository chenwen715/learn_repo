    ...
    public partial class Form1 : Form
    {
        ...
        public void GetSheetData()
        {
            ...
            {               
                ...
                richTextBox1.Text += DateTime.Now.ToString() + " 完成Excel数据读取\n";
        ...
        private void ManageUnLoadData()
        {
            ...
            for (int i = 0; i < totalNumber; i++)
            {
                ...
                unloadTask.pNo = dt.Rows[i][4].ToString();
                unloadTask.outNumber = Convert.ToInt32(dt.Rows[i][5].ToString());
                unloadTask.miniPerBag = Convert.ToInt32(dt.Rows[i][6].ToString());
...
　　　　                dtpnpb.Rows.Add(dr);  
        ...
        private void InsertSql(string sheetName, List<LoadData> list)
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                if (!string.IsNullOrEmpty(taskno))
                {
                    number = date + (Convert.ToInt16(taskno.Substring(taskno.Length - 4, 4)) + i + 1).ToString().PadLeft(8, '0');
                }
                else
                {
        ...
        private void InsertContainer(string sheetName, List<LoadData> list, List<LoadData> listall)
        {
            ...
            if (sheetName == "Pallet")
            {
                ...
                for (int i = 0; i < list.Count; i++)
                {
                    ...
                    while (list.Find(a => a.hubno == warecell) != null)
                    {
                        warecell = comnsql.ProcOutput(Properties.Settings.Default.sql1, "P_GetWareCell", paras, "@WareCell").ToString();
                    }
